...
<div class={['table-container relative rounded-3xl', getShadowClass(customShadow)]}>
							...
							{#each row.cells as cell, columnIndex}
								{#if cell.isHeader}
									...
									>
										...
										<div class={`cell-wrapper text-sm md:text-base flex items-center px-5.5 py-4.5 h-full md:${getJustifyClass(cell.alignment)}`}>
								...
								{:else} 
									...
									>
										...
										<div class="cell-wrapper flex justify-between items-center px-5.5 py-4 h-full md:{getJustifyClass(cell.alignment)}">
                                            {#if headers[columnIndex]}
                                                <span class="flex items-center font-medium text-themeGray-500 text-sm md:hidden">
                                                    {headers[columnIndex]}
                                                </span>
										    {/if}
											{#if cell.icon}
	...
	{#if showOverlay}
		<div class={`absolute pointer-events-none h-32 w-[calc(100%+14px)] -mx-[7px] -bottom-[7px] ${overlayType === 'gray' ? 'bg-gradient-to-b from-transparent to-[#f2f3fa]' : 'bg-gradient-to-b from-[rgba(242,243,250,0)] to-white'}`}></div>
	{/if}
...
<style>
	...
	.table :global {
		...
		tbody tr th:first-child, thead tr th:first-child { @apply md:pl-6; }
		tbody tr td:last-child { @apply md:pr-4; }
		...
		.status-success { @apply bg-[var(--color-themeGreen-100)] text-[var(--color-themeGreen-500)]; }
		.status-error { @apply bg-[var(--color-themeRed-100)] text-[var(--color-themeRed-500)]; }
		.status-warning { @apply bg-[var(--color-themeOrange-100)] text-[var(--color-themeOrange-500)]; }
		.status-check .icon { @apply p-0; }
