<script lang="ts">
	...
	let { title, description, icon, theme = 'primary', footerSlot, footerType, footerLink = '#', footerText = 'Learn more', ...restProps }: Props = $props();
...
<div class={['box relative flex h-full flex-col rounded-xl rounded-br-none drop-shadow-xl', 'px-8.5 pt-4 pb-9 md:px-12 md:pt-10 lg:px-15 lg:pt-12.5 lg:pb-4 lg:pb-11', theme === 'primary' ? 'text-themeGray-800 theme-gradient-yellow bg-gradient-to-t' : '', theme === 'secondary' ? 'bg-themeGray-800 text-white' : '', restProps.class]}>
	{#if showHeader}
	...
	{/if}
	<div class={['box__body theme-text-body2', theme === 'primary' ? 'text-themeGray-800 mb-6.5 md:mb-10' : '', theme === 'secondary' ? 'text-themeGray-200 mb-4 md:mb-8.5' : '']}>
		{#if description}
			{@html description}
		{/if}
	</div>
