<script lang="ts">
	...
	let { label, options = [], error, value = $bindable(''), showIcons = true, class: className = '', id, name }: Props = $props();
...
<div class="mb-4">
	<Select.Root type="single" onValueChange={handleValueChange} {value} items={options} onOpenChange={isOpen => (open = isOpen)}>
		<Select.Trigger class={['theme-gradient-white relative flex w-full cursor-pointer items-center rounded-lg bg-gradient-to-t px-4 py-3 text-left drop-shadow-md', error ? 'border border-red-500' : '', 'hover:border-themeYellow-600 transition-colors duration-200', className]} aria-label={label || 'Select an option'} {id} {name}>
