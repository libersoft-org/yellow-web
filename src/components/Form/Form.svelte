...
>
	...
	>
		<form onsubmit={handleSubmit}>
			<!-- Input -->
			<Input id="formInput" bind:value={inputValue} placeholder={m['form.input.placeholder']} label={m['form.input.label']} />

			<!-- Textarea -->
			<TextArea id="formTextarea" bind:value={textareaValue} placeholder={m['form.textarea.placeholder']} rows={5} label={m['form.textarea.label']} />
          ...
          font-medium">{m['form.select.label']}</label
				>
				<SelectBox id="formSelect" bind:value={selectValue} options={selectOptions} showIcons={false} label={m['form.select.placeholder']} />
			</div>
			...
			<div class="mb-4">
				<CheckBox id="consentCheckbox" bind:checked={consentValue} label={m['form.checkbox.label']} name="consent" />
			</div>
